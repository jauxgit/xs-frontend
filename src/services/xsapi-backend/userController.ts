...
/** getAkAndSk GET /api/user/get/aksk */
export async function getAkAndSkUsingGET(options?: { [key: string]: any }) {
  return request<API.BaseResponseUserAKVo>('/api/user/get/aksk', {
    method: 'GET',
    ...(options || {}),
  });
}
...
/** userResetPassword POST /api/user/resetPassword */
export async function userResetPasswordUsingPOST(
  body: API.UserResetPasswordRequest,
  options?: { [key: string]: any },
) {
  return request<API.BaseResponseboolean>('/api/user/resetPassword', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
/** updateUserAkAndSk POST /api/user/updateAkAndSk */
export async function updateUserAkAndSkUsingPOST(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.updateUserAkAndSkUsingPOSTParams,
  options?: { [key: string]: any },
) {
  return request<API.BaseResponseboolean>('/api/user/updateAkAndSk', {
    method: 'POST',
    params: {
      ...params,
    },
